...
scale = 50;                                        %rescale size for trainning model
dirName = uigetdir(pwd);                            %get the directory where image sets is at, starting path is the current path
Trainning_data = getAllFiles(dirName, selection , scale);   %get the trainned calss cell array by calling getAllFiles function

%**************************************************************************
result = [];
class = {};
l = length(cellfun('length', Trainning_data));     %get the length of the cell array, which is the number of classes in the image sets
for i = 1:l
    ...
    for j = 1:5
       ...
       result = getClass(file_path, Trainning_data ,scale);%result from getClass function.
